                      ...
                      return (
                        <td key={seg} className="p-2">
                          <select
                            ...
                            value={def?.role_id?.toString() || ""}
                            disabled={!monthlyEditing}
                            onChange={(e) => {
                              const rid = parseInt(e.target.value, 10);
                              setMonthlyDefault(p.id, seg, isNaN(rid) ? null : rid);
                            }}
                          >
                            <option value="">--</option>
    ...
    function RoleSelect({ month, personId, seg, def }: { month: string; personId: number; seg: Segment; def: any }){
      ...
      return (
        <select
          ...
          onChange={(e)=>{
            const rid = parseInt(e.target.value, 10);
            setMonthlyDefaultForMonth(month, personId, seg, isNaN(rid) ? null : rid);
            setDefs(all(`SELECT * FROM monthly_default`));
            showCode();
          }}
        >
          <option value=""></option>
          {options.map((r:any)=>(<option key={r.id} value={r.id.toString()}>{r.name}</option>))}
        </select>
      );
    }
