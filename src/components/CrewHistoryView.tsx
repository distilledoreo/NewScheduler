          ...
          default:
            if (segmentNames.includes(sortField as Segment)) {
              ...
              const defA = defs.find(
                (d) => d.month === month && d.person_id === a.id && d.segment === sortField
              );
              const defB = defs.find(
                (d) => d.month === month && d.person_id === b.id && d.segment === sortField
              );
                ...
                <React.Fragment key={p.id}>
                  {segList.map((seg, idx) => (
                    <tr key={`${p.id}-${seg}`} className="odd:bg-white even:bg-slate-50">
                      {idx === 0 && (
                        <td className="p-2 border border-slate-300" rowSpan={segList.length}>
