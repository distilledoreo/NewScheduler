...
import { Button, Checkbox, Dropdown, Input, Option, Button, Input, Dropdown, Option, Field, Checkbox } from "@fluentui/react-components";
  ...
  function RequiredCell({date, group, role, segment}:{date:Date|null; group:any; role:any; segment:Segment}){
    ...
    return (
      <div className="flex items-center gap-2">
        <Input type="number" value={val} onChange={(_, { value }) =>setVal(parseInt(e.target.value||'0',10))} 
        />
        <Button 
...
function PeopleEditor(){
          ...
          <div className="min-h-full flex items-start justify-center p-4">
            <div className="bg-white w-full max-w-3xl max-h-[85vh] overflow-auto rounded-xl p-4 shadow-xl">
              ...
              <div className="grid grid-cols-12 gap-2 mb-3">
                <Input placeholder="Last Name" value={form.last_name||''} onChange={(_, { value }) =>setForm({...form,last_name:e.target.value})} />
                <Input placeholder="First Name" value={form.first_name||''} onChange={(_, { value }) =>setForm({...form,first_name:e.target.value})} />
                <Input placeholder="Work Email" value={form.work_email||''} onChange={(_, { value }) =>setForm({...form,work_email:e.target.value})} />
                <Dropdown selectedOptions={[form.brother_sister || "Brother"]} onOptionSelect={(_, d) => setForm({ ...form, brother_sister: d.optionValue as string })}><Option value="Brother">Brother</Option><Option value="Sister">Sister</Option></Dropdown>
                <label className="col-span-2 flex items-center gap-2"><Checkbox checked={!!form.commuter} onChange={(_, { checked }) =>setForm({...form,commuter:e.target.checked})}/> Commuter</label>
                <label className="col-span-2 flex items-center gap-2"><Checkbox checked={form.active!==false} onChange={(_, { checked }) =>setForm({...form,active:e.target.checked})}/> Active</label>
                {WEEKDAYS.map((w,idx)=> (
  ...
  onOptionSelect={(_, d) => {
    ...
    setForm({ ...form, [key]: d.optionValue as string });
  }}
>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1 max-h-40 overflow-auto border rounded p-2">
                  {roles.map((r:any)=>(
                    <label key={r.id} className="flex items-center gap-1 text-xs">
                      <Checkbox checked={qualifications.has(r.id)} onChange={(_, { checked }) => {
